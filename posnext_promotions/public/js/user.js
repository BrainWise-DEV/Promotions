...
frappe.ui.form.on("User", {
	refresh(frm) {
		...
		frappe
			.call("posnext_promotions.api.authorization.has_authorization_pin", { user: frm.doc.name })
			.then((r) => {
...
function show_pin_dialog(frm, has_pin, is_self, pin_length, strict_pin_enforced) {
	...
	const dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			frappe
				.call("posnext_promotions.api.authorization.set_authorization_pin", {
					user: frm.doc.name,
...
function remove_pin(frm) {
	frappe.confirm(
		...
		() => {
			frappe
				.call("posnext_promotions.api.authorization.clear_authorization_pin", { user: frm.doc.name })
				.then(() => {
