...
``posnext_promotions.api.invoices.apply_offers`` is the *only* engine that runs for a POS
...
``validate`` hook in :mod:`posnext_promotions.overrides.pricing_rule`.
...
def run_line_discount_passes(items, rule_map, selected_offer_names=None, cap_resolver=None) -> set:
	...
	pass that runs afterwards (see :mod:`posnext_promotions.overrides.pricing_rule`).
	"""
	if not items:
		return set()
