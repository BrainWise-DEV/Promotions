...
# posnext_promotions.promotions.engine during apply_offers, not by this module.
...
def apply_price_discount_rule(pricing_rule, item_details, args):
	...
	- ``Accumulative`` — the percentage comes from the scope rows present across the
	  ...
	  is applied by ``posnext_promotions.promotions.engine``.
...
def apply_min_max_price_discounts(doc, method=None, allowed_rules=None):
	...
	When called as a ``doc_events`` hook (``method`` set) and ``pos_next`` is
	installed, skip — develop already runs the same hook. Direct calls from
	this app's ``apply_offers`` still run.
